...
class Router {
	...
	/**
	 * 该api 只对h5有效
	 * @param {Number} delta 需要前进的路由层级 默认1 
	 */
	go(delta=1){
		if(appPlatform() === 'H5'){
			return this._H5PushTo('go',delta);
		}
		warn(`非H5端调用 go() api 无效哦`);
	}
	/**
	 * 该api 只对h5有效
	 */
	forward(){
		if(appPlatform() === 'H5'){
			return this._H5PushTo('forward',undefined);
		}
		warn(`非H5端调用 forward() api 无效哦`);
	}
	/**
	 * @param {Object} Vim
	 * 
