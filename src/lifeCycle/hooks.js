...
export const resolveParams = async function(router, rule, fnType, navigateFun) {
	const _from = resolveRule(router, {
	...
	try {
		...
		if (beforeEnter) {
			fromatRule = await resolveRouterHooks(ags, beforeEnter, fromatRule.toRule);
		}
		navigateFun.call(router, fromatRule.toRule);
		
		await resolveAfterHooks(router,{
			...route(),
...
export const resolveRouterHooks = function({
	...
	return new Promise(async (resolve, reject) => {
		...
		if (Object.keys(fromatRule.ags).length > 0) {
			...
			if (beforeEnter) {
				return resolve(await resolveRouterHooks(fromatRule.ags, beforeEnter, fromatRule.toRule));
			}
		}
		resolve(fromatRule);
	})
